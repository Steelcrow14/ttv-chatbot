...
import { AudioSource, ChatBotCommand, CommandType, UserFile } from '@prisma/client';
...
const initNewCachedAudio = (
	...
	switch (command.audiosrc) {
		case AudioSource.file: {
			...
			const fileURL = process.env.NEXT_PUBLIC_DOMAIN?.concat('/api/downloads/', audioFile.id);
			if (!fileURL) {
				return undefined;
			}
			const newHowl = new Howl({
				src: [fileURL],
				format: audioFile.format ?? undefined,
				preload: 'metadata',
				html5: true,
				onend: function (): void {
